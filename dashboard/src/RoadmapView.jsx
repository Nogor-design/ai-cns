...
function RoadmapTaskModal({ task, onClose, onSave }) {
  ...
  return <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && onClose()}><form className="roadmap-task-modal" onSubmit={submit} aria-labelledby="roadmap-task-title">
    <div className="modal-head"><div><span className="eyebrow">Local schedule record</span><h2 id="roadmap-task-title">{task.title}</h2><p>{task.project_name} · {task.id}</p></div><button type="button" className="icon-button" onClick={onClose}><X size={18} /></button></div>
    ...
    <div className="roadmap-edit-note"><AlertTriangle size={14} /><span>These edits update local Cortex schedule evidence only. They do not start AI work, change GitHub, or claim a critical path.</span></div>
    <div className="modal-actions"><button type="button" onClick={onClose}>Cancel</button><button type="submit" className="primary" disabled={saving}><Save size={14} />{saving ? 'Saving…' : 'Save local schedule'}</button></div>
  </form></div>
}
...
export default function RoadmapView({ tasks, projects, onSaveTask }) {
  const [scope, setScope] = useState('recent')
  const [projectId, setProjectId] = useState('')
  const [selectedTask, setSelectedTask] = useState(null)
  const roadmap = useMemo(
    () => buildRoadmap(tasks, { scope, projectId }),
    [tasks, scope, projectId],
  )
  const mobileRows = [...roadmap.scheduled, ...roadmap.unscheduled]

  return <section className="roadmap-view">
    <div className="roadmap-heading"><div><span className="eyebrow">Portfolio schedule</span><h1>Roadmap</h1><p>Planned dates, recorded delivery history, and unscheduled work stay visibly distinct.</p></div><div className="roadmap-filters"><label>Project<select value={projectId} onChange={event => setProjectId(event.target.value)}><option value="">All non-archived projects</option>{projects.map(project => <option key={project.project_id} value={project.project_id}>{project.name}{project.status === 'paused' ? ' · paused' : ''}</option>)}</select></label><label>Scope<select value={scope} onChange={event => setScope(event.target.value)}><option value="recent">Active + last 30 days</option><option value="active">Active work only</option><option value="all">All recorded work</option></select></label></div></div>
